...
void batterylevel(float desiredTime) { //cpwr = Cortex power, epwr = Expander power
	ctime=desiredTime*(1+(1-powerlevelCortex));
...
task lcd { //For LCD Autonomous Selection
	...
	int rightbutton = 4;
	char tempString[50];
	while(batterydisplay==1) {
		sprintf(tempString, "%f", nImmediateBatteryLevel/1000.0);
		displayLCDCenteredString(1,tempString);
	}
	while(batterydisplay==0) {
...
task scheduler { //Schedules the startup of the bot and user-created tasks
	...
	stopTask(autoled);
	stopTask(lcd); //Stops autoselection code
	bLCDBacklight = false;
...
task autonomous()
{
	...
	case 0: //This is if we want to disable autonomous; when auto == 0, do nothing.
		break;
	case 1:
		if(powerlevelCortex==1 && powerlevelExpander==1) {
			move_straight(5,127);
		}
		break;
	case 2:
